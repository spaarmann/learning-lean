...
def Table.cartesianProduct (t1 : Table s1) (t2 : Table s2) : Table (s1 ++ s2) :=
  t1.flatMap fun r1 => t2.map r1.append

def List.without [BEq α] (source banned : List α) : List α :=
  source.filter fun r => !(banned.contains r)

-- This only changes the *type* of the row, the actual runtime data is
...
-- Similarly here, we only need this to change the type.
